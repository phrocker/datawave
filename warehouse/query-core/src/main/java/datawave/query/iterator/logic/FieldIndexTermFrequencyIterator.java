 ...
 * 
 */
public class FieldIndexTermFrequencyIterator implements SortedKeyValueIterator<Key,Value>, DocumentIterator {
    private static final Logger log = Logger.getLogger(FieldIndexTermFrequencyIterator.class);
     ...
     * @param source
     */
    public FieldIndexTermFrequencyIterator(Key fiStartKey, Key fiEndKey, SortedKeyValueIterator<Key,Value> source, TimeFilter timeFilter) {
        this(fiStartKey, fiEndKey, source, timeFilter, null, false, Predicates.<Key> alwaysTrue(), new TermFrequencyAggregator(null, null));
    }

    public FieldIndexTermFrequencyIterator(Key fiStartKey, Key fiEndKey, SortedKeyValueIterator<Key,Value> source, TimeFilter timeFilter, TypeMetadata typeMetadata,
    ...
    public FieldIndexTermFrequencyIterator(Range fiRange, SortedKeyValueIterator<Key,Value> source, TimeFilter timeFilter, TypeMetadata typeMetadata,
                                           boolean buildDocument, Predicate<Key> datatypeFilter, FieldIndexAggregator aggregator) {
    ...
    public FieldIndexTermFrequencyIterator(Key fiStartKey, boolean startInclusive, Key fiEndKey, boolean endInclusive, SortedKeyValueIterator<Key,Value> source,
    ...
    public void buildScanRangeLazily() {
        Key startKey = new Key(startKeyParser.getRow(), new Text(columnFamily ),
                new Text(startKeyParser.getFieldValue() + Constants.NULL + startKeyParser.getDataType() + Constants.NULL + startKeyParser.getUid()));
        ...
        Key endKey = new Key(stopKeyParser.getRow(), new Text(columnFamily),
                new Text(stopKeyParser.getFieldValue() + Constants.NULL + stopKeyParser.getDataType() + Constants.NULL + stopKeyParser.getUid()));

        this.scanRange = new Range(startKey, startInclusive, endKey, endInclusive);
        //this.scanRange = new Range(startKey, startInclusive, startKey.followingKey(PartialKey.ROW_COLFAM_COLQUAL), endInclusive);
    ...
    public void next() throws IOException {
        ...
        while (source.hasTop() && tk == null) {
            ...
            /*if (key.getFieldName().compareTo(field) < 0) {
                ...
                StringBuilder builder = new StringBuilder(key.getFieldValue()).append(Constants.NULL).append(key.getUid()).append(Constants.NULL)
