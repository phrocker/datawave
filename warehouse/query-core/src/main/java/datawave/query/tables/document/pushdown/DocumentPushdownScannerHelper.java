...
public class DocumentPushdownScannerHelper {

    public static DocumentPushdownScannerImpl createDocumentBatchScanner(AccumuloClient client, String tableName, Collection<Authorizations> authorizations, int numQueryThreads, Query query, boolean docRawFields, DocumentSerialization.ReturnType returnType, int queueCapacity, int maxTabletsPerRequest, int maxTabletThreshold) throws TableNotFoundException {
        DocumentPushdownScannerImpl batchScanner = null;
        if (authorizations != null && !authorizations.isEmpty()) {
            Iterator<Authorizations> iter = AuthorizationsMinimizer.minimize(authorizations).iterator();
            batchScanner = new DocumentPushdownScannerImpl((ClientContext)client, TableId.of((String)((ClientContext)client).tableOperations().tableIdMap().get(tableName)), tableName, (Authorizations)iter.next(), numQueryThreads, returnType, docRawFields, queueCapacity, maxTabletsPerRequest, maxTabletThreshold);
